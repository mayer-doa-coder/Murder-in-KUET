...
def test_simulate_move_returns_new_state_instance() -> None:
    state = _build_state()
    old_state = state
    new_state = state.simulate_move("Academic Building")

    assert old_state is not new_state
    assert old_state.current_player.position == "Library"
    assert new_state.current_player.position == "Academic Building"
...
def test_invalid_move_returns_clone_without_mutation() -> None:
    ...
    assert old_state is not new_state
    assert old_state.current_player.position == "Library"
    assert new_state.current_player.position == "Library"
