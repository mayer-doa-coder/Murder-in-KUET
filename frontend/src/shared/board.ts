// board.ts — Pure board geometry + pathfinding, shared by client rendering and
// the authoritative server's move validation. Ported verbatim from the original
// frontend hook (useBoard.ts) with no React dependency. This is the single
// source of truth for the board; the frontend re-exports these symbols.
...
export function cellWalkable(type: CellType): boolean {
...
// Inverse of DOOR_POSITIONS: which room (if any) a door cell belongs to.
export function doorRoomAt(col: number, row: number): string | null {
  for (const [roomId, doors] of Object.entries(DOOR_POSITIONS)) {
    if (doors.some(([dc, dr]) => dc === col && dr === row)) return roomId
  }
  return null
}
